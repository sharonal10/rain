...
def training(dataset, opt, pipe, testing_iterations ,saving_iterations, checkpoint_iterations ,checkpoint, debug_from, args_dict):
    ...
    assert len(gaussians_list[0].centers) == 1, len(gaussians_list[0].centers)
    assert len(gaussians_list[1].centers) == 3, len(gaussians_list[1].centers)

    with torch.no_grad(): # manually place the items
        gaussians_list[0].centers[0].data[:] = torch.tensor([0, 0, -0.022]).float().cuda()
        gaussians_list[0].scale.data = torch.tensor(0.957).float().cuda()
        ...
        gaussians_list[1].rot_vars[1].data = torch.tensor(-0.758).float().cuda()

        gaussians_list[1].centers[2].data[:] = torch.tensor([-0.265, 1.018, -0.077]).float().cuda()
        gaussians_list[1].rot_vars[2].data = torch.tensor(-0.758).float().cuda()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration % 1000 == 0 or iteration < 4 or iteration == 50:
            ...
            assert False # just render once then stop
