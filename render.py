...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, args_dict):
    with torch.no_grad():
        ...
        gaussians = GaussianModel(dataset.sh_degree, divide_ratio, scale=args_dict['scale'], mask_id=0)
        ...
        if args_dict['scale'] != 1:
            args_dict['prefix'] = args_dict['prefix'] + '_scale' + str(args_dict['scale']) + '_'
