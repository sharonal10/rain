...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, num_cams):
    with torch.no_grad():
        divide_ratio = 0.8
        gaussians = GaussianModel(dataset.sh_degree, divide_ratio)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, num_cams=num_cams)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.num_cams)
